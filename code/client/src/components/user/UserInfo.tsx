...
export const UserInfo: FC<{}> = observer(({}) => {
	const { sSignUp, sPartner } = useStore();
	...
	const handleGenderChange = (event: SelectChangeEvent) => {
		...
		sPartner.set_gender(+event.target.value);
	};
			...
			<FormControl sx={{ m: 1, width: 1 }}>
				...
				<OutlinedInput
					...
					onChange={(event) => {
						...
						sPartner.set_fullName(event.target.value);
					}}
					startAdornment={
				...
				<LocalizationProvider dateAdapter={AdapterDateFns}>
					<DesktopDatePicker
						...
						renderInput={(params: any) => {
							return <TextField name="dob" {...params} />;
						}}
					/>
			...
			<FormControl fullWidth sx={{ m: 1 }}>
				...
				<OutlinedInput
					...
					onChange={(event) => {
						...
						sPartner.set_address(event.target.value);
					}}
				/>
			</FormControl>
			<FormControl fullWidth sx={{ m: 1, width: "50ch" }}>
				...
				<OutlinedInput
					...
					onChange={(event) => {
						...
						sPartner.set_email(event.target.value);
					}}
					required
				/>
			</FormControl>
			<FormControl fullWidth sx={{ m: 1, width: "50ch" }}>
				...
				<OutlinedInput
					...
					onChange={(event) => {
						...
						sPartner.set_phone(event.target.value);
					}}
					required
				/>
			</FormControl>
